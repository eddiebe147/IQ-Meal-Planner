...
class MealPlannerApp {
    ...
    showUserSetup() {
        ...
        modalTitle.textContent = 'Welcome to Family Meal Planner!';
        ...
        modalBody.innerHTML = `
            <div class="user-setup">
                <div class="setup-header">
                    ...
                    <p>Create your profile to start planning meals and optionally invite your partner!</p>
                </div>
                
                <form class="profile-form" onsubmit="app.createUserProfile(event)">
                    <div class="form-group">
                        <label>Your Name</label>
                        <input type="text" id="userName" required placeholder="e.g., Sarah">
                    </div>
                    
                    <div class="form-group">
                        <label>Profile Emoji (optional)</label>
                        <input type="text" id="userEmoji" maxlength="2" placeholder="👩‍🍳">
                        <div class="form-hint">Pick an emoji that represents you!</div>
                    </div>
                    
                    <div class="form-group">
                        <label>Dietary Preferences (optional)</label>
                        <div class="preference-tags">
                            <div class="pref-tag" onclick="app.togglePreference(this, 'vegetarian')">🥗 Vegetarian</div>
                            <div class="pref-tag" onclick="app.togglePreference(this, 'vegan')">🌱 Vegan</div>
                            <div class="pref-tag" onclick="app.togglePreference(this, 'gluten-free')">🌾 Gluten-Free</div>
                            <div class="pref-tag" onclick="app.togglePreference(this, 'dairy-free')">🥛 Dairy-Free</div>
                            <div class="pref-tag" onclick="app.togglePreference(this, 'keto')">🥑 Keto</div>
                            <div class="pref-tag" onclick="app.togglePreference(this, 'low-carb')">🥩 Low-Carb</div>
                        </div>
                        <input type="hidden" id="userPreferences" value="[]">
                    </div>
    ...
    togglePreference(element, preference) {
        element.classList.toggle('active');
        
        // Update hidden input with selected preferences
        const activePrefs = document.querySelectorAll('.pref-tag.active');
        const preferences = Array.from(activePrefs).map(el => 
            el.onclick.toString().match(/'([^']+)'/)[1]
        );
        
        document.getElementById('userPreferences').value = JSON.stringify(preferences);
    }
    ...
    createUserProfile(event) {
        ...
        const emoji = document.getElementById('userEmoji').value || '👤';
        const preferences = JSON.parse(document.getElementById('userPreferences').value || '[]');
        ...
        this.data.familyMembers = [userProfile];
        
        this.saveData();
        this.closeModal();
        
        this.showNotification('🎉', 'Profile Created!', `Welcome to your family meal planner, ${name}!`);
        
        // Show family options after profile creation
        setTimeout(() => {
    ...
    showFamilyOptions() {
        ...
        modalTitle.textContent = '👨‍👩‍👧‍👦 Family Setup';
        
        modalBody.innerHTML = `
            <div class="family-options">
                <div class="family-header">
                    <h3>Ready to invite your partner?</h3>
                    <p>You can invite family members to collaborate on meal planning, or continue solo for now.</p>
                </div>
                
                <div class="family-choice">
                    <div class="choice-card" onclick="app.showInviteInterface()">
                        <div class="choice-icon">💕</div>
                        <h4>Invite My Partner</h4>
                        <p>Share meal planning with your significant other. Vote on meals together!</p>
                        <div class="choice-action">Set Up Family Account →</div>
                    </div>
                    
                    <div class="choice-card" onclick="app.continueSolo()">
                        <div class="choice-icon">🧑‍🍳</div>
                        <h4>Continue Solo</h4>
                        <p>Keep meal planning personal for now. You can always invite family later!</p>
                        <div class="choice-action">Start Planning →</div>
                    </div>
                </div>
            </div>
        `;
    ...
    continueSolo() {
        this.closeModal();
        this.showNotification('🍽️', 'Ready to Cook!', 'Your personal meal planner is ready. Check the settings to invite family later!');
    ...
    showInviteInterface() {
        ...
        modalTitle.textContent = '💕 Invite Your Partner';
        
        modalBody.innerHTML = `
            <div class="invite-interface">
                <div class="invite-header">
                    <div class="invite-icon">💌</div>
                    <h3>Invite ${this.data.user.emoji} Your Cooking Partner!</h3>
                    <p>Share your meal planning with someone special</p>
                </div>
                ...
                <div class="invite-methods">
                    <div class="invite-method">
                        ...
                        <div class="invite-link-container">
                            <input type="text" id="inviteLink" readonly value="${window.location.origin}${window.location.pathname}?invite=${this.data.family.inviteCode}" />
                            <button class="btn btn-secondary" onclick="app.copyInviteLink()">Copy Link</button>
                        </div>
                        <div class="invite-hint">Send this link to your partner via text, email, or any messaging app</div>
                    </div>
                    
                    <div class="invite-method">
                        <h4>💬 Share Invite Code</h4>
                        <div class="invite-code-display">
                            <span class="invite-code">${this.data.family.inviteCode}</span>
                            <button class="btn btn-secondary btn-small" onclick="app.copyInviteCode()">Copy</button>
                        </div>
                        <div class="invite-hint">They can enter this code in their meal planner app</div>
                    </div>
                </div>
                
                <div class="family-preview">
                    <h4>👨‍👩‍👧‍👦 Your Family Kitchen</h4>
                    <div class="family-name">${this.data.family.name}</div>
                    <div class="family-members">
                        <div class="member-card admin">
                            <span class="member-emoji">${this.data.user.emoji}</span>
                            <span class="member-name">${this.data.user.name}</span>
                            <span class="member-role">Admin</span>
                        </div>
                        <div class="member-card pending">
                            <span class="member-emoji">❓</span>
                            <span class="member-name">Waiting for partner...</span>
                ...
                <div class="invite-actions">
                    <button class="btn btn-secondary" onclick="app.closeModal()">Invite Later</button>
                    <button class="btn btn-primary" onclick="app.showInviteSuccess()">Link Copied!</button>
                </div>
            </div>
        `;
    ...
    copyInviteLink() {
        ...
        this.showNotification('📋', 'Link Copied!', 'Paste this link to invite your partner');
    }

    copyInviteCode() {
        navigator.clipboard.writeText(this.data.family.inviteCode);
        this.showNotification('📋', 'Code Copied!', `Invite code ${this.data.family.inviteCode} copied to clipboard`);
    }

    showInviteSuccess() {
        this.closeModal();
        this.showNotification('💕', 'Invitation Ready!', 'Share the link with your partner. They\'ll join your family kitchen!');
        this.updateTabContent(this.currentTab);
    }

    // Utility functions for IDs and codes
    generateUserId() {
        return 'user_' + Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
    }

    generateFamilyId() {
        return 'family_' + Math.random().toString(36).substr(2, 9) + Date.now().toString(36);
    }

    generateInviteCode() {
        const words = ['COOK', 'MEAL', 'CHEF', 'FOOD', 'YUMMY', 'TASTY', 'SPICE', 'FRESH'];
    ...
    // Update UI to show family status
    updateUI() {
        this.updateStats();
    ...
    updateFamilyDisplay() {
        ...
        const familyMembersDisplay = document.getElementById('familyMembersDisplay');
        
        if (this.data.user && this.data.family) {
            familyStatus.style.display = 'flex';
            familyName.textContent = this.data.family.name;
            
            const membersHTML = this.data.familyMembers.map(member => `
                <div class="family-member-mini">
                    <span class="member-emoji-mini">${member.emoji}</span>
                    <span class="member-name-mini">${member.name}</span>
                </div>
            `).join('');
            
            familyMembersDisplay.innerHTML = membersHTML;
        } else {
            familyStatus.style.display = 'none';
        }
    }

    // Family Settings Interface
    showFamilySettings() {
        const modalTitle = document.getElementById('modalTitle');
        const modalBody = document.getElementById('modalBody');
        
        modalTitle.textContent = '👨‍👩‍👧‍👦 Family Settings';
        
        modalBody.innerHTML = `
            <div class="family-settings">
                <div class="family-overview">
                    <h3>${this.data.family.name}</h3>
                    <div class="family-stats">
                        <div class="family-stat">
                            <span class="stat-number">${this.data.familyMembers.length}</span>
                            <span class="stat-label">Members</span>
                        </div>
                        <div class="family-stat">
                            <span class="stat-number">${this.data.recipes.length}</span>
                            <span class="stat-label">Shared Recipes</span>
                        </div>
                        <div class="family-stat">
                            <span class="stat-number">${Object.keys(this.data.mealPlans).length}</span>
                ...
                <div class="settings-section">
                    ...
                    <div class="members-list">
                        ${this.data.familyMembers.map(member => `
                                ...
                                <div class="member-profile">
                                    ...
                                    <div class="member-details">
                                        ...
                                        ${member.preferences && member.preferences.length > 0 ? 
                                            `<div class="member-prefs">${member.preferences.join(', ')}</div>` : ''
                                        }
                                    </div>
                                </div>
                ...
                <div class="settings-section">
                    <h4>📤 Invite More Members</h4>
                    <div class="invite-quick">
                        <div class="invite-code-quick">
                            <span class="quick-code">${this.data.family.inviteCode}</span>
                            <button class="btn btn-small btn-secondary" onclick="app.copyInviteCode()">Copy</button>
                        </div>
                        <button class="btn btn-primary" onclick="app.showInviteInterface()">Share Invite Link</button>
                    </div>
                </div>
                
                <div class="settings-section">
                    <h4>⚙️ Family Preferences</h4>
                    <div class="family-settings-grid">
                        <div class="setting-item">
                            <label class="setting-label">
                                <input type="checkbox" checked> Enable weekly voting
                            </label>
                            <div class="setting-desc">Members vote on meal choices democratically</div>
                        </div>
                        
                        <div class="setting-item">
                            <label class="setting-label">
                                <input type="checkbox" checked> Share calendar events
                            </label>
                            <div class="setting-desc">Add meal times to everyone's calendar</div>
                        </div>
                        
                        <div class="setting-item">
                            <label class="setting-label">
                                <input type="checkbox"> Allow takeout suggestions
                            </label>
                            <div class="setting-desc">Include "order takeout" as a meal option</div>
                        </div>
                    </div>
                </div>
                
                <div class="settings-actions">
                    <button class="btn btn-secondary" onclick="app.exportFamilyData()">📤 Export Data</button>
    ...
    // Takeout Logging System
    logTakeout() {
        const modalTitle = document.getElementById('modalTitle');
        const modalBody = document.getElementById('modalBody');
        
        modalTitle.textContent = '🍕 Log Takeout Order';
        
        modalBody.innerHTML = `
            <div class="takeout-logger">
                <div class="takeout-header">
                    <div class="takeout-icon">🍕</div>
                    <h3>Sometimes takeout is the best option!</h3>
                    <p>Log your order to keep track of your weekly meal balance</p>
                </div>
                
                <form class="takeout-form" onsubmit="app.saveTakeoutOrder(event)">
                    <div class="form-group">
                        <label>What did you order?</label>
                        <input type="text" id="takeoutMeal" required placeholder="e.g., Thai Green Curry, Pizza, Chinese">
                    </div>
                    
                    <div class="form-row">
                        <div class="form-group">
                            <label>Restaurant</label>
                            <input type="text" id="takeoutRestaurant" placeholder="Restaurant name">
                        </div>
                        <div class="form-group">
                            <label>Cost (optional)</label>
                            <input type="number" id="takeoutCost" step="0.01" placeholder="25.99">
                        </div>
                    </div>
                    
                    <div class="form-group">
                        <label>Which planned meal is this replacing?</label>
                        <select id="replacedMeal">
                            <option value="">Not replacing a planned meal</option>
                            ${this.getThisWeeksPlannedMeals().map(meal => 
                                `<option value="${meal.day}">${meal.day}: ${meal.recipe ? meal.recipe.name : 'Planned meal'}</option>`
                            ).join('')}
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="takeoutNotes" placeholder="Date night, too tired to cook, celebration, etc."></textarea>
                    </div>
                    
                    <div class="form-actions">
                        <button type="button" class="btn btn-secondary" onclick="app.closeModal()">Cancel</button>
                        <button type="submit" class="btn btn-primary">Log Takeout Order</button>
                    </div>
                </form>
            </div>
        `;
    ...
    getThisWeeksPlannedMeals() {
        const weekKey = this.getWeekKey(this.currentWeek);
        const mealPlan = this.data.mealPlans[weekKey] || {};
        
        return Object.entries(mealPlan).map(([day, meal]) => ({
            day: day.charAt(0).toUpperCase() + day.slice(1),
            recipe: this.data.recipes.find(r => r.id === meal.recipeId)
        }));
    }

    saveTakeoutOrder(event) {
        event.preventDefault();
        
        const takeoutOrder = {
            id: Date.now(),
            meal: document.getElementById('takeoutMeal').value,
            restaurant: document.getElementById('takeoutRestaurant').value,
            cost: parseFloat(document.getElementById('takeoutCost').value) || 0,
            replacedMeal: document.getElementById('replacedMeal').value,
            notes: document.getElementById('takeoutNotes').value,
            date: new Date().toISOString(),
            weekKey: this.getWeekKey(this.currentWeek),
            orderedBy: this.data.user.name
        };
        
        // Add to takeout log
        if (!this.data.takeoutLog) this.data.takeoutLog = [];
        this.data.takeoutLog.push(takeoutOrder);
        
        // If replacing a planned meal, mark it in meal plan
        if (takeoutOrder.replacedMeal) {
            const weekKey = this.getWeekKey(this.currentWeek);
            if (this.data.mealPlans[weekKey] && this.data.mealPlans[weekKey][takeoutOrder.replacedMeal.toLowerCase()]) {
                this.data.mealPlans[weekKey][takeoutOrder.replacedMeal.toLowerCase()].replacedWithTakeout = {
                    orderId: takeoutOrder.id,
                    meal: takeoutOrder.meal,
                    restaurant: takeoutOrder.restaurant
                };
            }
        }
        ...
        this.showNotification('🍕', 'Takeout Logged!', `${takeoutOrder.meal} from ${takeoutOrder.restaurant || 'restaurant'} added to your meal tracking`);
    }
    ...
    exportFamilyData() {
        const exportData = {
            ...
            mealPlans: this.data.mealPlans,
            takeoutLog: this.data.takeoutLog,
        ...
        a.href = url;
        a.download = `${this.data.family.name.replace(/\s+/g, '_')}_meal_data.json`;
        a.click();
        window.URL.revokeObjectURL(url);
        
        this.showNotification('📤', 'Data Exported!', 'Family meal data downloaded as JSON file');
    }
